# Building a Python client to make REST API calls to the Streams service
...
Included in the sample there is a configuration file with placeholders that need to be replaced with the proper values. They include information for authentication, your account information and connecting to the Streams service instance.
...
1. To modify the Stream Type definition in StreamType.json. If you change the properties to be different than timestamp and value, you'll need to modify the generate_random_timeindexeddouble_data function to handle your properties accordingly.
1. To modify the Stream definitions in Stream1.json and Stream2.json.
1. To modify the start and end time of data backfill and data reading as well as the behaviour for readig data, modify the global variables at the beginning of Program.py:
    ...
    - **DATA_READ_INTERVAL** - Indicates a time interval at which you'd like to read interpolated data (format is hh:mm:ss). This will be converted to a number of intervals to be sent to streams endpoint based on the start and end time.
